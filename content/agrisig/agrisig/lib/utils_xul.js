...
function CalculerNombreDeLigne(query)
{
	...
	{
	    enumerator.next();
	    CodeValue=enumerator.getVariant(0);
	}
    else
	{
...
function Init_ALeDroit()
{
    ...
    if (res.rowCount!=0)
	{
	    enum=res.enumerate();
	    enum.first();
	    SuperUser=enum.getVariant(0);
	    NomUser=enum.getVariant(1);
	}
    else
	{
	...
	if (SuperUser)
	    ...
	    {
		RemplirAvecDroit(TabDroit,NomUser,enum.getVariant(1));
	    }
