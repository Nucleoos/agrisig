...
// 	      <button onclick="svg_fin_detoure();">Fin DS3</button>
	...
	case "textes":
		carte_rafraichir_texte();//à appeler lors de la modification du nom d'un sous-îlot
...
//   // si on a trouvé la ligne, on la selectionne
...
function getOngletCourant()
{
	...
	//on récupère l'onglet séléctionné
...
// activer=false => on désactive tout les fils
...
function GererSuppr()
{
	...
	// on récupère la liste où sont affichées les valeures existantes
	var FrameConcernee= top.frames["Frame_"+NomOnglet];
	var lst = FrameConcernee.document.getElementById(id);
	var selecteditem=lst.selectedItem;
	if (selecteditem==null)
	{
		alert("Aucun(e) des "+NomOnglet+" n'est selectonné(e)");
	}
	else
	{
...
function GererAdd()
{
	...
	// on récupère le nombre de colonnes pour savoir le nbr de valeur à demander
	...
	if (tab==undefined)
		{
			...
			else
			{
				...
				/* on fait du onload pour la connexion à la BD */
