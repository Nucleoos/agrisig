...
function genererButtonInsertMajSupXul(Composant)
{
    ...
    Code+='<hbox  pack="end" align="stretch" class="fondstyle">\n';
    ...
    */
    Code+='    <button disabled="true" id="Insert_'+ParentXul+'" label="Ajouter" oncommand="Insert_'+ParentXul+'()" '+(!Composant.MonComposantVisuel.Can(INSERTION) ? "hidden='true'" : "")+'/>\n';
    Code+='    <button disabled="true" id="Delete_'+ParentXul+'" label="Supprimer" oncommand="Delete_'+ParentXul+'()" '+(!Composant.MonComposantVisuel.Can(SUPPRESSION) ? "hidden='true'" : "")+'/>\n';
    Code+='    <button disabled="true" id="Update_'+ParentXul+'" label="Modifier" oncommand="Update_'+ParentXul+'()" '+(!Composant.MonComposantVisuel.Can(MODIFICATION) ? "hidden='true'" : "")+'/>\n';
    /*
    ...
    */
	...
	{
	    Code+='    <button id="Validate_'+ParentXul+'" label="Valider" oncommand="Validate_'+ParentXul+'()"  disabled="true" />\n';
	    Code+='    <button id="Annuler_'+ParentXul+'" label="Annuler" oncommand="Annuler_'+ParentXul+'()"  disabled="true" />\n';
    ...
    if (Esclaves.length!=0)
	{
	    ...
	    if (TabStrOnglets.length!=0)
		{
		    ...
		    else
			{
			    ...
			    if (TabStrOnglets.length>=max_onglets) CodeSlaves+='<hbox style="overflow: auto;">\n';
			    CodeSlaves+='<tabs ';
			    if (TabStrOnglets.length>=max_onglets) CodeSlaves+=' height="60" ';
			    CodeSlaves+='>\n';
			    ...
			    CodeSlaves+='</tabs>\n';
			    if (TabStrOnglets.length>=max_onglets) CodeSlaves+='</hbox>\n';
...
clInterfaceSimple.prototype.GenererInterfaceXulPanel=function()
{
	...
	if (this.MesBoutons.length>0){
	    StrXul+='  <hbox flex="1" pack="start" align="center">\n';
	...
	if (this.Dependant)
	{
	    ...
	    StrXul+='    <button label="Retour à l\'onglet précédent" oncommand="Retour_'+NomOnglet+'()"/>\n';
	    StrXul+='  </hbox>\n';
	}
	StrXul+='</hbox>\n';
	//StrXul+='</groupbox>\n';
    ...
    function()
{
	...
	if (Compo.getTheme().HasInsertMajSup())
		...
		StrJsDebut+=' if (pgsql_update(Req)==0)\n';
		StrJsDebut+='	alert("Echec lors de la supression");\n';
		...
		StrJs+=' {\n';
		StrJs+=' 	alert("Vous devez selectionner l\'enregistrement à suprimer");\n';
		StrJs+=' 	return;\n';
		StrJs+=' }\n';
    ...
    function(MonNomFichier,overlay,TitreFenetre,AutoSave)
{
	...
	StrXul+='<?xml-stylesheet href="chrome://agrisig/skin/icons.css" type="text/css"?>\n';
	StrXul+='-->\n';
    }
    else
    {
    ...
    StrXul+='<!-- CE FICHIER A ETE GENERE PAR GENERATEUR_INTERFACE_LIB -->\n\n';

    if (overlay)
    StrXul+='<overlay\n';
    else
    StrXul+='<window sizemode="maximized" \n';
    ...
    StrXul+='<script src="'+GEN_PATH+'generateur_interface_sql.js"/>\n';
    StrXul+='<script src="'+GEN_PATH+'generateur_interface_xul.js"/>\n';
    ...
    StrJs+="if (result.rowCount>0){var enum=result.enumerate();enum.first();top.document.getElementById('status_login').label='Login : '+enum.getVariant(0);}\n";
